...
class DoublePositive:
        ...
        for di in [-1, 0, 1]:
            for dj in [-1, 0, 1]:
                for dk in [-1, 0, 1]:
                    if di == 0 and dj == 0 and dk == 0: continue
                    ni, nj, nk = i + di, j + dj, k + dk
                    if 0 <= ni < mix_mask.shape[0] and 0 <= nj < mix_mask.shape[1] and 0 <= nk < mix_mask.shape[2]:
                        if mix_mask[ni][nj][nk] != 0: return mix_mask[ni][nj][nk]
        return 0

    def de_noise(self):
        def has_surranding_positive(i, j, k):
            count = 0
            check_list = [-1, 1]
            #check_list = [-1, 1, -2, 2]
            for di in check_list:
                ni= i + di
                if 0 <= ni < self.image_shape[0] and self.raw_double_positive[ni][j][k] > 0: count += 1
            if count == len(check_list):return True
            return False
        ...
        for i in tqdm(range(self.image_shape[0])):
            for j in range(self.image_shape[1]):
                for k in range(self.image_shape[2]):
                    if self.raw_double_positive[i][j][k] > 0 and has_surranding_positive(i, j, k):
                        self.denoised_double_positive[i][j][k] = self.raw_double_positive[i][j][k]
    ...
    def remove_small_clusters(self):
        ...
        self.selected_idx = torch.where(self.keypoint_counts >= self.cluster_threshold)[0]

        for i in tqdm(range(self.image_shape[0])):
            for j in range(self.image_shape[1]):
                for k in range(self.image_shape[2]):
                    if self.keypoint_mask[i][j][k] not in self.selected_idx:
                        self.final_mask[i][j][k] = 0
                ...
                for j in range(self.image_shape[1]):
                    for k in range(self.image_shape[2]):
                        if self.raw_double_positive[i][j][k] > 0 and self.final_mask[i][j][k] == 0:
                            ...
                            if mask_val in self.selected_idx and mask_val != 0:
    ...
    def count_statistics(self):
        ...
        for i in tqdm(range(self.image_shape[0])):
            for j in range(self.image_shape[1]):
                for k in range(self.image_shape[2]):
                    if self.final_mask[i][j][k] != 0 and not self.final_mask[i][j][k] in self.background_idx_list:
                        self.total_volume_weighted += self.raw_double_positive[i][j][k]
